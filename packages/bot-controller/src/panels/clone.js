...
function build(data = {}) {
  ...
  return replyV2(
    container([
      ...
      actionRow([
        btn(
          cloneRoles    ? "🎭  Rôles ✅"      : "🎭  Rôles ❌",
          "clone:toggleRoles",
          cloneRoles    ? ButtonStyle.Success  : ButtonStyle.Secondary
        ),
        btn(
          cloneChannels ? "💬  Salons ✅"     : "💬  Salons ❌",
          "clone:toggleChannels",
          cloneChannels ? ButtonStyle.Success  : ButtonStyle.Secondary
        ),
        btn(
          cloneEmojis   ? "😀  Emojis ✅"     : "😀  Emojis ❌",
          "clone:toggleEmojis",
          cloneEmojis   ? ButtonStyle.Success  : ButtonStyle.Secondary
        ),
        btn(
          cloneSettings ? "⚙️  Params ✅"     : "⚙️  Params ❌",
          "clone:toggleSettings",
          cloneSettings ? ButtonStyle.Success  : ButtonStyle.Secondary
        ),
      ]),
      separator(),
      actionRow([
...
function buildRunning(data = {}) {
  ...
  const STEP_LABELS = {
    start:         "🚀 Démarrage",
    roles:         "🎭 Clonage des rôles",
    roles_done:    "🎭 Rôles ✅",
    channels:      "💬 Clonage des salons",
    channels_done: "💬 Salons ✅",
    emojis:        "😀 Clonage des emojis",
    emojis_done:   "😀 Emojis ✅",
    settings:      "⚙️ Application des paramètres",
    done:          "✅ Terminé",
    error:         "❌ Erreur",
  };
  ...
  // Déterminer l'étape courante normalisée (sans _done)
  const currentStepBase = step.replace("_done", "");
  const currentStepIdx  = STEP_ORDER.indexOf(currentStepBase);

  // Construire la checklist des étapes
  const stepsLine = STEP_ORDER.map((s, i) => {
    const icon = STEP_ICONS[s];
    if (i < currentStepIdx || step === `${s}_done` || step === "done") {
      return `\`✅\` ${icon}`;
    }
    if (s === currentStepBase && step !== `${s}_done`) {
      return `\`⏳\` ${icon}`;
    }
    return `\`⬜\` ${icon}`;
  }).join("  ");
  ...
  const pctLine  = total > 0
    ? `\`${bar}\` **${current}/${total}** *(${Math.round((current / total) * 100)}%)*`
    : `\`🔄\` Démarrage…`;
...
function buildHistory(data = {}) {
  ...
  const list = history.length
    ? [...history].reverse().map((entry) => {
        ...
        if (entry.cancelled) {
          emoji  = "🛑";
          detail = `*Annulé manuellement*`;
        } else if (entry.success) {
          emoji  = "✅";
          detail = `🎭 ${entry.rolesCloned ?? 0}  ·  💬 ${entry.channelsCloned ?? 0}  ·  😀 ${entry.emojisCloned ?? 0}  ·  ⏱️ ${entry.duration ?? 0}s`;
        } else {
          emoji  = "❌";
          detail = `⚠️ ${entry.error ?? "Erreur"}`;
        }
        const time = new Date(entry.timestamp).toLocaleString("fr-FR");
        return (
          `${emoji} **${entry.sourceGuildName ?? entry.sourceGuildId}** → **${entry.targetGuildName ?? entry.targetGuildId}**\n` +
          `> 🕐 ${time}\n` +
          `> ${detail}`
        );
      }).join("\n\n")
  ...
  return replyV2(
    container([
      ...
      separator(),
      actionRow([
        btn("🗑️  Effacer l'historique", "clone:clearHistory", ButtonStyle.Danger),
      ]),
      separator(),
...
module.exports = { build, buildRunning, buildResult, buildHistory, buildGuildList };